  ...
  actions: {
    ...
    addTab(router: Router) {
      ...
      if (!this.tabList.length) {
        if (!sessionStorage.getItem('lylist')) {
          // @ts-ignore
          obj.push(tabItem)
          sessionStorage.setItem('lylist', JSON.stringify(obj))
        } else {
          obj = JSON.parse(sessionStorage.getItem('lylist') as string)
        }
        this.tabList = obj
      } else {
        this.tabList.push(tabItem)
        sessionStorage.setItem('lylist', JSON.stringify(this.tabList))
      }
      this.tabList.forEach((item: any) => {
        this.tasMap[item.fullPath] = item
      })
    },
